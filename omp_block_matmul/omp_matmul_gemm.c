...
void do_block (int bi, int bj, int bk, float *A, float *B, float *C, int len_i, int len_k, int len_j) {
  float *D , *E , *F;
  D = (float *) malloc( sizeof(float) * BLOCKSIZE * BLOCKSIZE );
  E = (float *) malloc( sizeof(float) * BLOCKSIZE * BLOCKSIZE );
  F = (float *) malloc( sizeof(float) * BLOCKSIZE * BLOCKSIZE ); // oh my god. copying this is for later!
  int i,k,j;
  for (i=0; i<BLOCKSIZE; i++){
    for (j=0; j<BLOCKSIZE; j++){
      for (k=0; k<BLOCKSIZE; k++){
      D[i*BLOCKSIZE+k] = A[(bi+i)*len_k + bk + k];
      E[k*BLOCKSIZE+j] = B[(bk+k)*len_j + bj + j];
      //C[(bi+i)*len_j + bj + j] += A[(bi+i)*len_k + bk + k ]*B[(bk+k)*len_j + bj + j];
      }
    }
  }
  cblas_sgemm ( CblasRowMajor, CblasNoTrans, CblasNoTrans, BLOCKSIZE, BLOCKSIZE, BLOCKSIZE, 1.0 , D , BLOCKSIZE, E, BLOCKSIZE , 0.0, F, BLOCKSIZE);
  for (i = 0; i < BLOCKSIZE; i++) {
    for (j = 0; j < BLOCKSIZE; j++) {
  	C[(bi+i)*len_j + bj+j] = F[i*BLOCKSIZE + j];
     }
  }
}
