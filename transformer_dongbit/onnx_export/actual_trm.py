...
model.apply(initialize_weights) #그냥 이런 식으로 apply 하는구나.
...
'''
src = torch.load("src.pt") #colab에서 빼옴
trg = torch.load("trg.pt")
torch.onnx.export(model,(src,trg[:,:-1]),'trm.onnx', opset_version=14) #model forward에 맞게 pass해줘야함.

#saving src, trg.pt as text

import numpy as np
np.savetxt('src.txt',src.numpy())
np.savetxt('trg.txt',trg.numpy())
