...
class electra(nn.Module):
    def __init__(self):
        ...
        self.tokenizer = ElectraTokenizerFast.from_pretrained("google/electra-large-discriminator") #identical to Bert tokenizer fake_sentence
        self.discriminator = ElectraForPreTraining.from_pretrained("google/electra-large-discriminator")
...
#with profile(on_trace_ready=torch.profiler.tensorboard_trace_handler('./log/electra_large'),profile_memory=True,with_stack=True,record_shapes=True) as prof: #for tensorboard
    #with record_function("model_inference"):
fake_sentence = "The quick brown fox fake over lazy dog " *  16 #길이 128로 맞추기. (need to check if this is all inputted.) + if padded.
test_model = electra()
predictions = test_model(fake_sentence)
