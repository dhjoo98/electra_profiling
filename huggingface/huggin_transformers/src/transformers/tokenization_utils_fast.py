...
class PreTrainedTokenizerFast(PreTrainedTokenizerBase):
    ...
    def __init__(self, *args, **kwargs):
        tokenizer_object = kwargs.pop("tokenizer_object", None) #하나씩 빼내온다.
    ...
    def tokenize(self, text: str, pair: Optional[str] = None, add_special_tokens: bool = False, **kwargs) -> List[str]: #하위 상속에서 정의한 vocabfile을 가지고 올라가나?
        return self.encode_plus(text=text, text_pair=pair, add_special_tokens=add_special_tokens, **kwargs).tokens() #그냥 그대로 pass해준다. (for sake of portability)
        #arg pair is optional for multi sequence inputs.
        #encode_plus의 tokens()만을 빼온다. 
    ...
    def _encode_plus( #_function_name means it is intended for internal use.
        self,
