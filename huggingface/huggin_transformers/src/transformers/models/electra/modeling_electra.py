...
class ElectraModel(ElectraPreTrainedModel):
    def __init__(self, config):
        ...
        self.encoder = ElectraEncoder(config) #this must be the encoder  = small version of BERT
        self.config = config
...
class ElectraForPreTraining(ElectraPreTrainedModel): #여기가 이제 돌리는데 crucial한 파트.
    def __init__(self, config):
        super().__init__(config) # take in config.

        self.electra = ElectraModel(config) #from
        self.discriminator_predictions = ElectraDiscriminatorPredictions(config) #from
        # Initialize weights and apply final processing
        self.post_init()
    ...
    def forward(
        ...
        ```
        # 출력값: tensor([[-10.5815,  -1.7485,  -3.2617,  -3.1303,  -2.2885,  -3.7960,  -2.3473, -10.5884]], grad_fn=<SqueezeBackward1>)
        이걸 이제 후처리에 넣어줘야한다. 
        """
        return_dict = return_dict if return_dict is not None else self.config.use_return_dict
