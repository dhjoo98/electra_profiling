...
#with profile(on_trace_ready=torch.profiler.tensorboard_trace_handler('./log/electra'),profile_memory=True,with_stack=True,record_shapes=True) as prof: #for tensorboard
    with record_function("model_inference"):
        ...
        # 소스 문장에 따른 마스크 생성
        src_mask = model.make_src_mask(src_tensor) #aten::ne and more.

        # 인코더(endocer)에 소스 문장을 넣어 출력 값 구하기
        with torch.no_grad(): #disable gradient calculation.
        #useful when sure that tensor.backward() will not be called.
        #thus will reduce memory consumption.
            enc_src = model.encoder(src_tensor, src_mask)
            #class Encoder init만으로 자동으로 forward를 call하나?: nn.Module에서 계승받은 __call__ 성질, 객체 호출로 forward를 바도 실행한다. 
